...
class Holidays extends Controller {
    ...
    function Holidays(){
        ...
        $this->load->model('holidays_model', 'holidaysProvider');
    ...
    function index(){
        $body['holidays'] = $this->holidaysProvider->Get(NULL, $this->school_id);	//$this->session->userdata('schoolcode'));
    ...
    */
    function edit($id = NULL){
        ...
        $content0['holiday'] = $this->holidaysProvider->Get($id, $this->school_id);
    ...
    function save(){
        ...
        $startString = $this->input->post("date_start");
        $endString = $this->input->post("date_end");

        $explodeStart = explode('/', $startString);
        $explodeEnd = explode('/', $endString);

        $startDate = new DateTime($explodeStart[2]."-".$explodeStart[1]."-".$explodeStart[0]);
        $endDate = new DateTime($explodeEnd[2]."-".$explodeEnd[1]."-".$explodeEnd[0]);
        ...
        if ($this->validation->run() == FALSE || !$dateValidation){
            if (!$dateValidation) {
                $this->session->set_flashdata('error', $this->load->view('msgbox/error', 'The dates you entered were invalid. Dates must be in the format dd/mm/yyyy, and the start date must be before the end date.', true) );
            }
            echo $holiday_id;
            // Validation failed
            if($holiday_id != "X"){
            ...
            if($holiday_id == 'X'){
                ...
                $holiday_id = $this->holidaysProvider->Add($data);
                ...
                $this->holidaysProvider->Edit($holiday_id, $data);
